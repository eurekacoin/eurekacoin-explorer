...
angular.module('insight.contracts')
	...
	.factory('Contracts',
	function(EurekaCoinCoreLib, Opcodes, Networks, Constants) {

		var CONTRACT_CALL = 194;
		var CONTRACT_CREATE = 193;

		return {
			isValidEurekaCoinAddress: function (address) {
                return EurekaCoinCoreLib.Address.isValid(address, Constants.NETWORK)
			},
			getBitAddressFromContractAddress: function (contractAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        ...
                        checksum = EurekaCoinCoreLib.crypto.Hash.sha256sha256(new EurekaCoinCoreLib.deps.Buffer(networkId + contractAddress, 'hex')),
                        hexBitAddress = networkId + contractAddress + checksum.toString('hex').slice(0, 8);

                    return EurekaCoinCoreLib.encoding.Base58.encode(new EurekaCoinCoreLib.deps.Buffer(hexBitAddress, 'hex'));
            ...
            getEthAddressFromBitAddress: function (bitAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        networkId = network.pubkeyhash.toString(16),
                        hexBitAddress = EurekaCoinCoreLib.encoding.Base58.decode(bitAddress).toString('hex');
			...
			getContractOpcodesString: function (hex) {
				...
				var contractCode = new EurekaCoinCoreLib.deps.Buffer(hex, 'hex'),
					ops = [];
			...
			getContractBytecode: function (hex) {
				...
				try {
					...
					var script = EurekaCoinCoreLib.Script(hex);
			...
			getContractAddress: function (txId, num) {
				var reverseTxId = txId.match(/.{2}/g).reverse().join(""),
					buf = new EurekaCoinCoreLib.deps.Buffer(4);

				buf.writeUInt32LE(num, 0);

				var nHex = buf.toString('hex'),
					addr = reverseTxId + nHex,
					bufferAddress = EurekaCoinCoreLib.crypto.Hash.sha256ripemd160(new EurekaCoinCoreLib.deps.Buffer(addr, 'hex'));
